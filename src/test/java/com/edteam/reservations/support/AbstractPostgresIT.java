...
public abstract class AbstractPostgresIT {
    ...
    protected long countAudit(String action) {
        Long count = jdbcTemplate.queryForObject("SELECT count(*) FROM auditoria WHERE accion = ?", Long.class, action);
    ...
    protected long countOutbox(String status) {
        Long count =
                jdbcTemplate.queryForObject("SELECT count(*) FROM outbox_message WHERE status = ?", Long.class, status);
        return count == null ? 0L : count;
    }
