...
class AirportCatalogPropertiesTest {
    ...
    private static AirportCatalogProperties withBaseUrl(String baseUrl) {
        return new AirportCatalogProperties(null, null, null, null, null, null, null, null, null, baseUrl, null, null);
    }

    @Test
    @DisplayName("exige HTTPS hacia el proveedor: la API key viaja en un header")
    void requiresHttpsForRemoteCatalogs() {
        assertThat(withBaseUrl("https://catalog.example/api").usesSecureTransport())
                .isTrue();
    ...
    void allowsLocalhostOverPlainHttp() {
        ...
        assertThat(withBaseUrl("http://localhost:6070/api").usesSecureTransport())
                .isTrue();
        assertThat(withBaseUrl("http://127.0.0.1:6070/api").usesSecureTransport())
                .isTrue();
    }
