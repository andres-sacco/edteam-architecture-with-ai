...
class FailuresTest {
    ...
    class Catalog {
        ...
        void transientFailuresCountAndRetry() {
            FailureClassification classification =
                    Failures.catalog(new AirportCatalogUnavailableException("503"));
        ...
        void throttlingCountsButDoesNotRetry() {
            ...
            FailureClassification classification =
                    Failures.catalog(new AirportCatalogThrottledException("429"));
        ...
        void permanentFailuresNeitherCountNorRetry() {
            ...
            FailureClassification classification =
                    Failures.catalog(new AirportCatalogIntegrationException("401"));
        ...
        void bulkheadFullCountsAndDoesNotRetry() {
            FailureClassification classification = Failures.catalog(
                    BulkheadFullException.createBulkheadFullException(
                            Bulkhead.of("x", BulkheadConfig.ofDefaults())));
        ...
        void ourOwnBugsNeverOpenTheCircuit() {
            assertThat(Failures.catalog(new IllegalArgumentException("código vacío")).countsForCircuit())
                    .isFalse();
        }
    }
    ...
    class Cache {
        ...
        void everyLettuceFailureCounts() {
            FailureClassification classification =
                    Failures.cache(new RedisConnectionFailureException("caído"));
        ...
        void ourOwnMistakesArePermanent() {
            assertThat(Failures.cache(new IllegalArgumentException("clave nula")).countsForCircuit()).isFalse();
        }
    }
    ...
    class Broker {
        ...
        void brokerProblemsCount() {
            assertThat(Failures.broker(new EventPublishException("no confirmó")).countsForCircuit()).isTrue();
        }
        ...
        void aReturnedMessageNeverOpensTheCircuit() {
            ...
            assertThat(Failures.broker(new EventRoutingException("sin binding")).countsForCircuit()).isFalse();
        }

        @Test
        @DisplayName("el rechazo del propio circuito no cuenta")
        void theCircuitDoesNotCountItsOwnRejections() {
            assertThat(Failures.broker(new EventPublisherUnavailableException("circuito abierto")).kind())
        ...
        void aSerializationFailureIsPermanent() {
            assertThat(Failures.broker(new IllegalStateException("payload inválido")).kind())
        ...
        void nothingIsRetriedInProcess() {
            assertThat(Failures.broker(new EventPublishException("no confirmó")).retryable()).isFalse();
            assertThat(Failures.broker(new EventRoutingException("sin binding")).retryable()).isFalse();
        }
    }
}
